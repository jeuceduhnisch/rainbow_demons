...
- Record trigger tip -> B10. Successive rising clock edges perform
  start-recording / stop-and-play / start-new-recording.
...
7. In Slice or Scatter, send three REC trigger pulses: pulse 1 starts capture,
   pulse 2 stops for playback, and pulse 3 starts a fresh capture.
8. In Scatter, verify Time, Feedback and Filter run reverse-left, stop near
   noon and run forward-right with Direction centered.
9. Change modes and return; the old capture must not resume.
10. In Slice, turn Feedback fully CCW and confirm automatic recording is off.
