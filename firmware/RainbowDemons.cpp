...
bool capture_from_trigger = false;
...
void ClearMemory()
{
    ...
    phrase_generation = 0;
    capturing = false;
    capture_from_trigger = false;
    tape_valid_samples = 0;
    previous_tape_length = 0;
    ResetHeads();
}

void BeginCapture(bool from_trigger)
{
    capture_position = 0;
    phrase_samples = 0;
    phrase_ready = false;
    phrase_generation = 0;
    capturing = true;
    capture_from_trigger = from_trigger;
    ResetHeads();
}
                   ...
                   size_t size)
{
    ...
    if(c.mode != Mode::Tape)
    {
        if(record_pressed && !previous_record_pressed) BeginCapture(false);
        if(record_trigger) BeginCapture(true);
        if(!record_pressed && previous_record_pressed) EndCapture();
        // A trigger starts a bounded capture when there is no physical hold/release.
        if(record_trigger && !record_pressed) capturing = true;
    }
    previous_record_pressed = record_pressed;

    const bool reset_trigger = patch.gate_in_2.Trig();
    if(reset_button.RisingEdge() || reset_trigger) ResetHeads();
    ...
    for(size_t i = 0; i < size; ++i)
    {
        ...
        // A trigger has no release edge, so give it the native mode maximum.
        const size_t trigger_limit = c.mode == Mode::Slice
                                         ? static_cast<size_t>(4.0f * kSampleRate)
                                         : static_cast<size_t>(8.0f * kSampleRate);
        if(capturing && capture_from_trigger && capture_position >= trigger_limit)
            EndCapture();
